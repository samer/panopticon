...
color[] moodRingData = new color[10];  //Array of colors to store values from MoodRing
...
float emotionalQuotientData = 0.0;  //value to store data from EmotionalQuotient messages
...
void sendMoodRingMessage(int j) {
...
void randomizeSpiritCenterData() {
  float spiritVarFactor = .05;
...
void randomizeEmotionalQuotientData() {
  if (emoUp) {     //emo walking up
    emotionalQuotientData = emotionalQuotientData + random(0, .05);      
...
void randomizeBrainwaveData() {
	...
	brainwaveData = randomSentences[int(random(randomSentences.length))];
